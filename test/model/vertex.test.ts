import { EdgeModel, VertexModel, IVertexModel } from '../../src';
...
describe('Vertex - 节点', () => {
  ...
  describe.only('clone 节点', () => {
    ...
    it('默认拷贝的节点数据和原来保持一致', () => {
      const cloned = vertexA.clone();

      expect(vertexA.id).toBe('A');
      expect(vertexA.edges.length).toBe(1);
      expect(vertexA.edges[0].id).toBe('A_B');
      expect(vertexA.meta).toEqual({
        hello: 'world'
      });
      expect(cloned.toJSON()).toEqual(vertexA.toJSON());
    });
    ...
    it('拷贝时可指定 clone 函数', () => {
      ...
      expect(cloned1.id).toBe('A1');
      expect(cloned1.edges.length).toBe(1);
      ...
      expect(cloned2.edges.length).toBe(1);
